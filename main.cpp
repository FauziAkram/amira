...
struct Position {
    uint64_t piece_bb[6];
    uint64_t color_bb[2];
    Piece board[64]; // NEW: Mailbox array for O(1) piece lookup
    int side_to_move;
    int ep_square;
    ...
    Position() {
        ...
        // NEW: Explicitly initialize the board array
        for (int i = 0; i < 64; ++i) {
            board[i] = NO_PIECE;
        }
        side_to_move = WHITE;
    ...
    // OPTIMIZED: This is now an O(1) lookup
...
Position make_move(const Position& pos, const Move& move, bool& legal_move_flag) {
    ...
    if (piece_moved == PAWN && move.to == pos.ep_square && pos.ep_square != -1) {
        ...
        // NEW: Update mailbox for EP capture
    ...
    if (move.promotion != NO_PIECE) {
        ...
        // NEW: Update mailbox for promotion
        ...
        next_pos.color_bb[stm] |= to_bb;
        // NEW: Update mailbox for normal move
    ...
    if (piece_moved == KING) {
        ...
        if (std::abs(move.to - move.from) == 2) { // King moved two squares
            ...
            // NEW: Update mailbox for castling rook
    ...
    for (char c : part) {
        if (std::isdigit(c)) {
            ...
            if (p_type != NO_PIECE && rank >=0 && rank < 8 && file >=0 && file < 8) {
                ...
                // NEW: Update mailbox array during FEN parsing
                pos.board[sq] = p_type;
            }
            file++;
        }
