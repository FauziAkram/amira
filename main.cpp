...
const int connected_pawn_bonus_by_rank_mg[8] = {0, 8, 12, 20, 35, 60, 100, 0};
const int connected_pawn_bonus_by_rank_eg[8] = {0, 12, 18, 28, 45, 70, 110, 0};
const int defended_passed_pawn_bonus_mg[8] = {0, 4, 8, 16, 30, 50, 75, 0};
const int defended_passed_pawn_bonus_eg[8] = {0, 8, 15, 25, 45, 65, 90, 0};
...
const int gapped_doubled_pawn_penalty_mg = -8;
const int gapped_doubled_pawn_penalty_eg = -12;
const int hindered_pawn_penalty_mg = -8;
const int hindered_pawn_penalty_eg = -12;
const int pawn_threat_on_piece_bonus_mg = 25;
const int pawn_threat_on_piece_bonus_eg = 15;
const int pawn_push_threat_bonus_mg = 15;
const int pawn_push_threat_bonus_eg = 10;
...
void evaluate_pawn_structure_for_color(const Position& pos, Color current_eval_color,
    ...
    // Pawn Threats
    uint64_t friendly_pawn_attacks = 0;
    temp_pawns = all_friendly_pawns;
    while(temp_pawns) {
        friendly_pawn_attacks |= pawn_attacks_bb[current_eval_color][lsb_index(temp_pawns)];
        temp_pawns &= temp_pawns - 1;
    }
    uint64_t enemy_major_minor = pos.color_bb[1 - current_eval_color] & ~(pos.piece_bb[PAWN] | pos.piece_bb[KING]);
    mg_pawn_score += pop_count(friendly_pawn_attacks & enemy_major_minor) * pawn_threat_on_piece_bonus_mg;
    eg_pawn_score += pop_count(friendly_pawn_attacks & enemy_major_minor) * pawn_threat_on_piece_bonus_eg;

    // Pawn Push Threats
    uint64_t single_pushes = (current_eval_color == WHITE) ? north(all_friendly_pawns) : south(all_friendly_pawns);
    single_pushes &= ~pos.get_occupied_bb();
    uint64_t pushed_pawn_attacks = 0;
    temp_pawns = single_pushes;
    while(temp_pawns) {
        pushed_pawn_attacks |= pawn_attacks_bb[current_eval_color][lsb_index(temp_pawns)];
        temp_pawns &= temp_pawns - 1;
    }
    mg_pawn_score += pop_count(pushed_pawn_attacks & enemy_major_minor) * pawn_push_threat_bonus_mg;
    eg_pawn_score += pop_count(pushed_pawn_attacks & enemy_major_minor) * pawn_push_threat_bonus_eg;
    ...
    while (temp_pawns) {
        ...
        int rank = sq / 8;
        int file = sq % 8;
        int rank_from_own_side = (current_eval_color == WHITE) ? rank : (7 - rank);

        // Isolated Pawn Evaluation
        if ((adjacent_files_mask[file] & all_friendly_pawns) == 0) {
        ...
        // Connected Pawn (Phalanx) Bonus (rank-dependent)
        if (get_bit(east(set_bit(sq)), all_friendly_pawns)) {
            mg_pawn_score += connected_pawn_bonus_by_rank_mg[rank_from_own_side];
            eg_pawn_score += connected_pawn_bonus_by_rank_eg[rank_from_own_side];
        }
        
        // Doubled Pawn Evaluation
        if (pop_count(file_bb_mask[file] & all_friendly_pawns) > 1) {
            if ((current_eval_color == WHITE && get_bit(all_friendly_pawns, sq-8)) ||
                (current_eval_color == BLACK && get_bit(all_friendly_pawns, sq+8))) {
                mg_pawn_score += doubled_pawn_liability_mg;
                eg_pawn_score += doubled_pawn_liability_eg;
            } else {
                mg_pawn_score += gapped_doubled_pawn_penalty_mg;
                eg_pawn_score += gapped_doubled_pawn_penalty_eg;
            }
        }
        ...
        uint64_t adjacent_pawns = adjacent_files_mask[file] & all_friendly_pawns;
        ...
        if (is_passed) {
            ...
            // Defended Passed Pawn Bonus
            if (pawn_attacks_bb[1 - current_eval_color][sq] & all_friendly_pawns) {
                mg_pawn_score += defended_passed_pawn_bonus_mg[rank_from_own_side];
                eg_pawn_score += defended_passed_pawn_bonus_eg[rank_from_own_side];
            }
        }
    }
}
