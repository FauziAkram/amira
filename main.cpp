...
#include <cctype>
#include <cmath>
#include <map> // --- NEW --- For easier parameter management
...
// --- NEW --- Parameter Management Structure
struct TunableParam {
    std::string name;
    int value;
    int default_val;
...
void init_parameters() {
    ...
    AllParams[IsolatedPawnPenaltyMG.name] = &IsolatedPawnPenaltyMG;
    AllParams[IsolatedPawnPenaltyEG.name] = &IsolatedPawnPenaltyEG;
    AllParams[ProtectedPawnBonusMG.name] = &ProtectedPawnBonusMG;
    AllParams[ProtectedPawnBonusEG.name] = &ProtectedPawnBonusEG;
    AllParams[DoubledPawnPenaltyMG.name] = &DoubledPawnPenaltyMG;
    AllParams[DoubledPawnPenaltyEG.name] = &DoubledPawnPenaltyEG;
    AllParams[HinderedPawnPenaltyMG.name] = &HinderedPawnPenaltyMG;
    AllParams[HinderedPawnPenaltyEG.name] = &HinderedPawnPenaltyEG;
    AllParams[PassedPawnScaleMG.name] = &PassedPawnScaleMG;
    AllParams[PassedPawnScaleEG.name] = &PassedPawnScaleEG;
    AllParams[PassedPawnKingDistEG.name] = &PassedPawnKingDistEG;
    AllParams[KnightMobilityMG.name] = &KnightMobilityMG;
    AllParams[KnightMobilityEG.name] = &KnightMobilityEG;
    AllParams[BishopMobilityMG.name] = &BishopMobilityMG;
    AllParams[BishopMobilityEG.name] = &BishopMobilityEG;
    AllParams[RookMobilityMG.name] = &RookMobilityMG;
    AllParams[RookMobilityEG.name] = &RookMobilityEG;
    AllParams[QueenMobilityMG.name] = &QueenMobilityMG;
    AllParams[QueenMobilityEG.name] = &QueenMobilityEG;
    AllParams[BishopPairMG.name] = &BishopPairMG;
    AllParams[BishopPairEG.name] = &BishopPairEG;
    AllParams[KnightOutpostMG.name] = &KnightOutpostMG;
    AllParams[KnightOutpostEG.name] = &KnightOutpostEG;
    AllParams[BishopOutpostMG.name] = &BishopOutpostMG;
    AllParams[BishopOutpostEG.name] = &BishopOutpostEG;
    AllParams[PotentialOutpostBonus.name] = &PotentialOutpostBonus;
    AllParams[RookOpenFileMG.name] = &RookOpenFileMG;
    AllParams[RookOpenFileEG.name] = &RookOpenFileEG;
    AllParams[RookSemiOpenFileMG.name] = &RookSemiOpenFileMG;
    AllParams[RookSemiOpenFileEG.name] = &RookSemiOpenFileEG;
    AllParams[MinorOnHeavyPressureMG.name] = &MinorOnHeavyPressureMG;
    AllParams[MinorOnHeavyPressureEG.name] = &MinorOnHeavyPressureEG;
    AllParams[RookOnMinorPressureMG.name] = &RookOnMinorPressureMG;
    AllParams[RookOnMinorPressureEG.name] = &RookOnMinorPressureEG;
    AllParams[KingDangerScaleMG.name] = &KingDangerScaleMG;
    AllParams[KingDangerScaleEG.name] = &KingDangerScaleEG;
    AllParams[PawnShieldBonus.name] = &PawnShieldBonus;
    AllParams[PawnShieldPenalty.name] = &PawnShieldPenalty;
...
// King safety penalty tables (index is king_attack_score, capped). These are now base values for scaling.
const int king_danger_penalty_base_mg[15] = {0, 0, 5, 15, 30, 50, 75, 100, 130, 160, 200, 240, 280, 320, 350};
const int king_danger_penalty_base_eg[15] = {0, 0, 0,  5, 10, 15,  20,  30,  40,  50,  60,  70,  80,  90, 100};
...
// --- Evaluation ---
// --- MODIFIED --- All hardcoded tables and values are now base values.
// The actual values used in eval are derived from these and the TunableParams.
...
// Base values for passed pawn bonuses, to be scaled by tunable parameters
const int passed_pawn_bonus_base_mg[8] = {0, 5, 15, 25, 40, 60, 80, 0};
const int passed_pawn_bonus_base_eg[8] = {0, 10, 25, 40, 60, 90, 120, 0};
...
// --- MODIFIED --- This entire function is now refactored to use the TunableParams
int evaluate(const Position& pos) {
    ...
    // --- NEW --- Create local arrays for piece values from TunableParams for easier access.
    ...
    for (int c_idx = 0; c_idx < 2; ++c_idx) {
            ...
            while (b) {
                ...
                if ((Piece)p == PAWN) {
                    ...
                    if (is_passed) {
                        ...
                        mg_score += side_multiplier * (passed_pawn_bonus_base_mg[rank_from_own_side] * EvalParams::PassedPawnScaleMG.value / 100);
                        eg_score += side_multiplier * (passed_pawn_bonus_base_eg[rank_from_own_side] * EvalParams::PassedPawnScaleEG.value / 100);
                    ...
                    mg_mobility_score += side_multiplier * mobility_count * EvalParams::RookMobilityMG.value;
                    eg_mobility_score += side_multiplier * mobility_count * EvalParams::RookMobilityEG.value;
                }
                else if ((Piece)p == KNIGHT) {
                    uint64_t mobility_attacks = knight_attacks_bb[sq];
                    piece_attacks_bb[c_idx][KNIGHT] |= mobility_attacks;
                    int mobility_count = pop_count(mobility_attacks & attackable_squares);
                    mg_mobility_score += side_multiplier * mobility_count * EvalParams::KnightMobilityMG.value;
                    eg_mobility_score += side_multiplier * mobility_count * EvalParams::KnightMobilityEG.value;
                    int rank = sq / 8;
                    ...
                    int mobility_count = pop_count(mobility_attacks & attackable_squares);
                    mg_mobility_score += side_multiplier * mobility_count * EvalParams::BishopMobilityMG.value;
                    eg_mobility_score += side_multiplier * mobility_count * EvalParams::BishopMobilityEG.value;
                    int rank = sq / 8;
                    int relative_rank_idx = (current_eval_color == WHITE) ? rank : 7 - rank;
                     if (relative_rank_idx >= 3 && relative_rank_idx <= 5) {
                        if (get_bit(pawn_attacks_bb[enemy_color][sq], all_friendly_pawns)) {
                    ...
                    int mobility_count = pop_count(mobility_attacks & attackable_squares);
                    mg_mobility_score += side_multiplier * mobility_count * EvalParams::QueenMobilityMG.value;
                    eg_mobility_score += side_multiplier * mobility_count * EvalParams::QueenMobilityEG.value;
        ...
        if (king_sq != -1) {
            ...
            mg_score -= side_multiplier * (king_danger_penalty_base_mg[danger_idx] * EvalParams::KingDangerScaleMG.value / 100);
            eg_score -= side_multiplier * (king_danger_penalty_base_eg[danger_idx] * EvalParams::KingDangerScaleEG.value / 100);
        }
    }
...
// --- MODIFIED --- This entire function is refactored to use the parameter map
void uci_loop() {
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (token == "uci") {
            std::cout << "id name Amira 1.2 Tunable\n";
...
int main(int argc, char* argv[]) {
    ...
    init_parameters(); // --- NEW --- Must initialize the parameter map
    init_zobrist();
    init_attack_tables();
    init_eval_masks();
    reset_killers_and_history();
