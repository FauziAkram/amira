...
const PhaseScore piece_phase_values[6] = {
    {78, 135}, {384, 410}, {390, 430}, {573, 680}, {1127, 1350}, {0, 0}
};
const int see_piece_values[7] = {91, 319, 329, 506, 926, 10000, 0};
...
const PhaseScore king_pst[64] = {
    ...
    { 58,  37}, {107,  65}, { 73,  94}, {  3, 102}, { 20, 100}, { 74,  93}, {108,  67}, { 49,  30},
    { 58,  41}, {105,  65}, { 55, 108}, { 10, 110}, { -0, 110}, { 52, 107}, { 95,  68}, { 49,  41},
    { 59,  28}, { 92,  94}, { 58, 110}, { -1, 109}, { -2, 110}, { 50, 110}, { 86,  94}, { 51,  26},
...
const PhaseScore passed_pawn_bonus[8] = {
    {0, 0}, {14, 12}, {13, 28}, {23, 42}, {39, 65}, {56, 95}, {65, 125}, {0, 0}
};

// --- Evaluation Constants ---
const PhaseScore TEMPO_BONUS                   = {17, 17};
const PhaseScore BISHOP_PAIR_BONUS             = {45, 66};
const PhaseScore PROTECTED_PAWN_BONUS          = {0, 17};
const PhaseScore ISOLATED_PAWN_PENALTY         = {-12, -16};
const PhaseScore DOUBLED_PAWN_PENALTY          = {-4, -11};
const PhaseScore BACKWARD_PAWN_PENALTY         = {-15, -24};
const PhaseScore KNIGHT_MOBILITY_BONUS         = {11, 5};
const PhaseScore BISHOP_MOBILITY_BONUS         = {1, 15};
const PhaseScore ROOK_MOBILITY_BONUS           = {2, 5};
const PhaseScore QUEEN_MOBILITY_BONUS          = {5, 10};
const PhaseScore KNIGHT_OUTPOST_BONUS          = {32, 25};
const PhaseScore BISHOP_OUTPOST_BONUS          = {41, 19};
const PhaseScore POTENTIAL_DOMINANCE_BONUS     = {13, 3};
const PhaseScore ROOK_ON_OPEN_FILE             = {26, 15};
const PhaseScore ROOK_ON_SEMI_OPEN_FILE        = {15, 3};
const PhaseScore RookOnEnemyTerritoryBonus     = {12, 27};
const PhaseScore ConnectedRooksOnTerritoryBonus  = {9, 10};
const int PasserMyKingDistance[8] = {0, 1, 9, -15, 23, 29, 18, 0};
const int PasserEnemyKingDistance[8] = {0, 1, 3, 0, 29, 43, 30, 0};
...
const PhaseScore THREAT_BY_MINOR[7] = {
    {0,0}, {12,9}, {20,12}, {12,14}, {17,32}, {29,40}, {0,0} // Pawn, Knight, Bishop, Rook, Queen, King
};
const PhaseScore THREAT_BY_ROOK[7] = {
    {0,0}, {3,11}, {0,17}, {31,14}, {24,9}, {30,9}, {0,0}
};
const PhaseScore THREAT_BY_KING = {7, 27};
const PhaseScore HANGING_PIECE_BONUS = {6, 6};
const PhaseScore WEAK_QUEEN_DEFENSE_BONUS = {1, 15};
const PhaseScore RESTRICTED_PIECE_BONUS = {0, 3};
const PhaseScore SAFE_PAWN_ATTACK_BONUS = {32, 12};
const PhaseScore PAWN_PUSH_THREAT_BONUS = {5, 10};
...
const int SHIELD_PAWN_PRESENT_BONUS = 0;
const int SHIELD_PAWN_MISSING_PENALTY = -15;
const int SHIELD_PAWN_ADVANCED_PENALTY = -11;
const int SHIELD_OPEN_FILE_PENALTY = -16;
const int SafetyKnightWeight    = 33;
const int SafetyBishopWeight    = 17;
const int SafetyRookWeight      = 24;
const int SafetyQueenWeight     = 37;
const int SafetyAttackValue     = 34;
const int SafetyWeakSquares     = 44;
const int SafetySafeQueenCheck  = 67;
const int SafetySafeRookCheck   = 49;
const int SafetySafeBishopCheck = 54;
const int SafetySafeKnightCheck = 53;
const int SafetyAdjustment      = -68;
    ...
    for (int c_idx = 0; c_idx < 2; ++c_idx) {
        ...
        if (king_sq != -1) {
            ...
            if (king_attackers_count[current_eval_color] > 1) {
                ...
                current_color_score.eg -= safety_score / 14;
            }
        }
...
constexpr int TACTICAL_LOOKAHEAD_REDUCTION = 10;
...
void reset_search_heuristics() {
    ...
    for (int d = 1; d < MAX_PLY; ++d) {
        for (int m = 1; m < 256; ++m) {
            ...
            double reduction = (log(d) * log(m)) / 2.43;
...
int search(Position& pos, int depth, int alpha, int beta, int ply, bool is_pv_node, bool can_null_move, const Move& prev_move) {
    ...
    if (!is_pv_node && !in_check) {
        ...
        if (depth < 8) {
            ...
            int rfp_margin = 71 * (depth - improving);
        ...
        // Razoring
        if (depth < 4) {
            int razoring_margin = 183 + 85 * depth;
    ...
    if (!is_pv_node && !in_check && can_null_move && depth >= 3 && ply > 0 &&
        ...
        static_eval >= beta) {
            ...
            int R_nmp = 6 + depth / 7 + eval_bonus;
